local scriptDir = function()
	local path = debug.getinfo(1, "S").source:match("@(.*[\\/])")
	return path
end
local file_io = require(scriptDir() .. "utils/file_io")
...
local genArrHeaders = function(arr)
	...
	local c_buff = ""
	c_buff = c_buff .. '#include "led_timings.h"\n\n'
...
local main = function()
	...
	local abs_txt_src = scriptDir() .. rel_txt_src

	local rel_h_dest = "../include/led_timings.h"
	local abs_h_dest = scriptDir() .. rel_h_dest
	local rel_c_dest = "../src/led_timings.c"
	local abs_c_dest = scriptDir() .. rel_c_dest
